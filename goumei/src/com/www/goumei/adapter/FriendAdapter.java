...
public class FriendAdapter extends BaseAdapter {
	...
	public FriendAdapter(Context context, List<FriendData> list2) {
	...
	public View getView(final int position, View view, ViewGroup rg) {
		...
		if (view == null) {
			...
			holder.id = (TextView) view.findViewById(R.id.fansId);
		...
		if (bean.getHeadsculpture() != null) {
			imageLoader.displayImage(bean.getHeadsculpture(), holder.icon,
					options);
		}
	...
	class AddAttentionTask extends AsyncTask<String, String, String> {
		...
		protected void onPostExecute(String result) {
			...
			if (code != null && code.equals("1")) {
				...
				intent.setAction(Constant.ACTION_FIND_FRIEND);
				intent.putExtra(Constant.INTENT_INDEX, index);
				intent.putExtra(Constant.INTENT_STATUS,
						Constant.INTENT_STATUS_TRUE);
