...
public class FanFragment extends BaseFragment implements OnClickListener {
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		titleTv.setText("粉丝");
		adapter = new FansAdapter(pActivity, list);
	...
	class GetVideosTask extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... arg0) {
			videoList = ApiClientC.getFans(SP_ID.id, pageNo);
				...
				public void run() {
					...
					if (index >= 0) {
						if (Constant.INTENT_STATUS_TRUE.equals(status)) {
							list.get(index).setIsAttention(1);
						} else {
							list.get(index).setIsAttention(0);
						}
